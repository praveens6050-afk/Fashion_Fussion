'use strict';
(()=>{
  if(window.__ffAdminProductImageUploadReady)return;
  window.__ffAdminProductImageUploadReady=(async()=>{
    const readiness=window.ffAdminSupabaseReady||window.ffSupabaseReady;
    const client=readiness?await readiness:window.supabaseClient;
    if(!client)throw new Error('Admin services are not ready for image uploads.');
    if(window.__ffAdminProductImageUpload)return client;
    ...
    window.__ffAdminProductImageUpload=false;
    window.__ffAdminProductImageUploadReady=null;
    console.error('[Admin] Product image upload startup failed',error);
  });
})();
