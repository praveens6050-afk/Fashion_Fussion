...
window.fetch=async function(input,init){try{const url=typeof input==='string'?input:String(input?.url||'');if(init?.method==='POST'&&(/\/api\/(?:quote-order|create-order)(?:\?|$)/.test(url))&&typeof init.body==='string'){const parsed=JSON.parse(init.body);injectVariants(parsed);init={...init,body:JSON.stringify(parsed)}}}catch(e){console.warn('Variant checkout adapter skipped request enrichment',e)}return originalFetch(input,init)};
