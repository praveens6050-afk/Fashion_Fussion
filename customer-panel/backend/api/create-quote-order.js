...
async function rpc(name, args) {
  ...
  const data = await response.json().catch(() => null);
  if (!response.ok) {
    const error = new Error(data?.message || data?.error || 'Checkout operation failed');
    error.status = response.status;
    throw error;
  }
  return data;
...
function cleanAddress(a) {
  if (!a) throw new Error('Please add a delivery address before checkout.');
  const required = [a.full_name, a.phone, a.address_line1, a.city, a.state, a.postal_code, a.country];
  if (required.some(v => !String(v || '').trim())) throw new Error('Please complete your delivery address before checkout.');
  if (!/^\d{6}$/.test(String(a.postal_code).trim())) throw new Error('Please use a valid 6-digit delivery PIN code.');
  return {
    id: a.id || null,
    label: String(a.label || 'Home').trim(),
    full_name: String(a.full_name).trim(),
    phone: String(a.phone).trim(),
    address_line1: String(a.address_line1).trim(),
    address_line2: a.address_line2 ? String(a.address_line2).trim() : null,
...
async function loadQuote(userId, quoteId) {
  const rows = await rest(
    ...
    '&select=id,user_id,business_name,gstin,business_billing_address,status,quoted_subtotal,quoted_gst,quoted_delivery,quoted_total,valid_until,accepted_at&limit=1'
  );
  return rows?.[0] || null;
}
...
async function reserveQuoteInventory(orderId) {
  try {
...
async function failQuoteCheckout(orderId) {
  await rpc('release_order_inventory', { p_order_id: orderId }).catch(() => null);
...
async function finalizeQuoteCod(orderId, userId, source) {
  await rpc('finalize_cod_order_inventory', {
    p_order_id: orderId,
    p_user_id: userId,
    p_source: source
  });
  ...
  const response = await fetch('https://api.razorpay.com/v1/orders', {
    ...
    body: JSON.stringify({
      ...
      notes: {
        ...
        user_id: userId,
        store_order_id: String(storeOrder.id),
        bulk_quote_id: String(quoteId)
      },
...
async function attachRazorpayOrder(order, userId, razorpayOrder) {
  const rows = await rest(
    'orders?id=eq.' + encodeURIComponent(order.id) +
    '&user_id=eq.' + encodeURIComponent(userId) +
    '&status=in.(creating,created)&razorpay_order_id=is.null',
    {
      method: 'PATCH',
  ...
  const current = await loadOrder(userId, order.id);
  if (current?.status === 'created' && current.razorpay_order_id) return current;
  const error = new Error('This business payment could not be attached safely. Please open My Orders before retrying.');
  error.status = 409;
  throw error;
}
...
async function resumeExisting(order, userId, quoteId) {
  ...
  if (['payment_failed', 'expired', 'cancelled', 'cod_cancelled', 'refund_initiated', 'refund_pending', 'refund_failed', 'refunded'].includes(order.status)) {
  ...
  try {
    await reserveQuoteInventory(order.id);
  } catch (error) {
    await failQuoteCheckout(order.id);
    throw error;
  }
  ...
  if (order.payment_method === 'cod') {
    await finalizeQuoteCod(order.id, userId, 'quote_cod_resume');
    return { ...existingResponse({ ...order, status: 'cod_pending' }), completed: true };
  }

  let razorpayOrder;
  try {
    razorpayOrder = await createRazorpayOrder(order, userId, quoteId);
  } catch (error) {
    await failQuoteCheckout(order.id);
    error.status = error.status || 502;
    throw error;
  }
  const attached = await attachRazorpayOrder(order, userId, razorpayOrder);
  return {
    ...existingResponse(attached),
    key_id: KEY_ID,
    order: {
      id: attached.razorpay_order_id,
      amount: Math.round(Number(attached.total_amount) * 100),
      currency: attached.currency || 'INR'
    }
  };
}
  ...
  try {
    ...
    if (existing) {
      if (existing.payment_method !== paymentMethod) {
        return json(req, res, 409, { error: 'Payment method changed for this accepted quote.' });
      }
    ...
    const quotedTotal = Number(quote.quoted_total);
    const quotedSubtotal = Number(quote.quoted_subtotal);
    const quotedGst = Number(quote.quoted_gst);
    const quotedDelivery = Number(quote.quoted_delivery);
    if (![quotedTotal, quotedSubtotal, quotedGst, quotedDelivery].every(Number.isFinite) || quotedTotal <= 0 || quotedSubtotal < 0 || quotedGst < 0 || quotedDelivery < 0) {
      throw new Error('Quote pricing snapshot is invalid.');
    }

    const normalized = normalizeQuoteItems(await loadQuoteItems(quoteId));
    if (
      Math.abs(normalized.subtotal - quotedSubtotal) > 0.01 ||
      Math.abs(normalized.gst - quotedGst) > 0.01 ||
      Math.abs(roundMoney(normalized.subtotal + normalized.gst + quotedDelivery) - quotedTotal) > 0.01
    ) {
    ...
    const orderInsert = {
      ...
      business_billing_address: quote.business_billing_address || null,
    ...
    try {
      await reserveQuoteInventory(saved.id);
    } catch (error) {
      await failQuoteCheckout(saved.id);
      throw error;
    }

    if (paymentMethod === 'cod') {
      await finalizeQuoteCod(saved.id, user.id, 'quote_cod');
      return json(req, res, 200, {
        ...
        bulk_quote_id: quoteId,
        items: normalized.items,
        pricing: { subtotal: quotedSubtotal, gst: quotedGst, delivery: quotedDelivery, total: payment.total },
        total: payment.total
      });
    }
    ...
    let razorpayOrder;
    try {
      razorpayOrder = await createRazorpayOrder(saved, user.id, quoteId);
    } catch (error) {
      await failQuoteCheckout(saved.id);
      return json(req, res, 502, { error: error.message || 'Razorpay order creation failed' });
    }

    const attached = await attachRazorpayOrder(saved, user.id, razorpayOrder);
    return json(req, res, 200, {
      key_id: KEY_ID,
      store_order_id: attached.id,
      display_order_id: attached.display_order_id,
    ...
    console.error('create-quote-order error:', { message: error.message, status: error.status || null });
    return json(req, res, error.status || 400, { error: error.message || 'Unable to create quote order' });
  }
};
