...
function readBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on("data", chunk => {
      ...
      if (size > MAX_BODY_BYTES) { fail(new Error("Request body is too large")); req.destroy(); return; }
      body += chunk;
    });
    req.on("end", () => {
      if (settled) return;
      try { settled = true; resolve(JSON.parse(body || "{}")); }
...
async function getSupabaseUser(req) {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) throw new Error("Please log in before checkout");
  const accessToken = authHeader.substring(7).trim();
  const response = await fetch(SUPABASE_URL + "/auth/v1/user", {
    headers: { apikey: SUPABASE_SERVICE_ROLE_KEY, Authorization: "Bearer " + accessToken }
  });
  const data = await response.json();
  if (!response.ok || !data?.id) throw new Error("Your login session has expired. Please log in again.");
  return data;
}
...
async function calculate(items) {
  ...
  const finalAmount = subtotal - totalDiscount + totalGst + delivery + otherCharges;
  if (!Number.isFinite(finalAmount) || finalAmount < 1) throw new Error("Invalid order amount");
...
module.exports = {
  ...
  cors, json, readBody, basicAuth, getSupabaseUser, getProductsByIds, calculate
};
