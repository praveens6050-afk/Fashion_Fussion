...
module.exports = async function verifyPayment(req, res) {
  ...
  try {
    ...
    if (order.razorpay_payment_id && String(order.razorpay_payment_id) !== String(razorpay_payment_id)) {
      await recordPaymentException(order, 'different_payment_reference', razorpay_payment_id, {
      ...
      return json(req, res, 409, {
        ...
        captured: true,
        manual_review: true,
        error: 'This order is already linked to a different captured payment. Support review is required; please do not pay again.',
        store_order_id: order.id,
    ...
    if (order.status === 'paid') {
      try {
        ...
        console.error('Inventory commit repair failed:', error);
        return json(req, res, 409, {
          verified: false,
          captured: true,
          recoverable: true,
          error: 'Payment is captured. Inventory finalization is being reconciled; please do not pay again.',
          store_order_id: order.id,
          display_order_id: order.display_order_id
        });
      }
    ...
    if (isInactiveCheckoutStatus(order.status)) {
      console.error('Captured payment requires manual review for inactive order', {
        store_order_id: order.id,
        status: order.status,
        payment_id: razorpay_payment_id
      });
      await recordPaymentException(order, 'inactive_order_capture', razorpay_payment_id, {
        razorpay_order_id: String(order_id)
      });
      return json(req, res, 409, {
        verified: false,
        captured: true,
        manual_review: true,
        error: 'Payment is captured, but this order is no longer in an active checkout state. Support review is required; please do not pay again.',
        store_order_id: order.id,
        display_order_id: order.display_order_id,
        status: order.status
      });
    ...
    try {
      ...
      return json(req, res, 409, {
        ...
        captured: true,
        recoverable: true,
        error: 'Payment is captured. Finalization is pending reconciliation; please do not pay again.',
        store_order_id: order.id,
        display_order_id: order.display_order_id
      });
    }
