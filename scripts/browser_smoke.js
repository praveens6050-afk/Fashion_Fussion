...
(async () => {
  ...
  await journeyPage.route('**/rest/v1/products*', async route => {
    ...
    const body = accept.includes('application/vnd.pgrst.object+json') ? JSON.stringify(smokeProduct) : JSON.stringify([smokeProduct]);
    await route.fulfill({ status: 200, headers: { 'content-type': 'application/json; charset=utf-8', 'content-range': '0-0/1' }, body });
  });

  await visit('/search.html?q=Smoke', [['#grid', 'journey search results']], journeyPage);
  await journeyPage.locator('#grid .product').waitFor({ state: 'visible', timeout: 10000 });
  if (!(await journeyPage.locator('#grid').textContent()).includes('Smoke Test Product')) throw new Error('journey search did not render mocked product');
  ...
  if ((await journeyPage.locator('#name').textContent()) !== 'Smoke Test Product') throw new Error('journey PDP did not preserve selected product');
  ...
  if ((await journeyPage.locator('[data-qty="900001"]').inputValue()) !== '1') throw new Error('journey cart did not restore persisted quantity');
  const priceText = await journeyPage.locator('#prices').textContent();
  for (const expected of ['₹250', '₹45', '₹49']) if (!priceText.includes(expected)) throw new Error(`journey cart missing expected price component ${expected}`);
  ...
  if (Number(cartAfterQuantityChange['900001']) !== 2) throw new Error('journey cart quantity change did not persist quantity 2');
  const checkoutKeyAfterMutation = await journeyPage.evaluate(key => sessionStorage.getItem(key), CHECKOUT_KEY);
  if (checkoutKeyAfterMutation !== null) throw new Error('journey cart mutation did not invalidate stale checkout key');
  const updatedPriceText = await journeyPage.locator('#prices').textContent();
  for (const expected of ['₹500', '₹90', 'FREE', '₹590']) if (!updatedPriceText.includes(expected)) throw new Error(`journey cart recalculation missing ${expected}`);
  ...
  if (Number(cartAfterLoginRedirect['900001']) !== 2) throw new Error('journey cart state was lost while redirecting signed-out checkout to login');
  ...
  const checkoutProduct = { id: 900002, name: 'Authenticated Checkout Product', category: 'Smoke Department', price: 250, image_url: null, is_active: true };
  const checkoutAddress = { id: 77, label: 'Home', full_name: 'Smoke Customer', phone: '9999999999', address_line1: '1 Test Street', address_line2: null, city: 'Jaipur', state: 'Rajasthan', postal_code: '302001', country: 'India', is_default: true, created_at: '2026-01-01T00:00:00.000Z' };

  await checkoutContext.addInitScript(({ cartKey }) => {
    if (location.pathname.endsWith('/checkout.html')) localStorage.setItem(cartKey, JSON.stringify({ '900002': 1 }));
  }, { cartKey: CART_KEY });
    ...
    const stub = `
      (() => {
        ...
        function query(table){
          const q={select(){return q},in(){return q},eq(){return q},order(){return q},async maybeSingle(){if(table==='profiles')return {data:{full_name:'Smoke Customer',phone:'9999999999'},error:null};return {data:null,error:null}},then(resolve,reject){const data=table==='products'?[product]:table==='customer_addresses'?[address]:[];return Promise.resolve({data,error:null}).then(resolve,reject)}};
          return q;
        }
  ...
  await checkoutPage.route('https://fashion-fussion-olive.vercel.app/api/**', async route => {
    ...
    if (url.pathname.endsWith('/quote-order')) {
      if (payload.items?.[0]?.id !== 900002 || payload.items?.[0]?.qty !== 1) throw new Error('authenticated checkout quote did not preserve cart items');
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ pricing: { subtotal: 250, gst: 45, delivery: 49, coupon_discount: 0, gift_card_discount: 0, total: 344, payment_method: payload.payment_method || 'prepaid' } }) });
      return;
    }
    if (url.pathname.endsWith('/create-order')) {
      ...
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ store_order_id: 880001, display_order_id: 'FF-SMOKE-880001', payment_method: 'cod', status: 'cod_pending', completed: true, total: 344 }) });
      return;
    }
  ...
  if (!(await checkoutPage.locator('#addressBox').textContent()).includes('Smoke Customer')) throw new Error('authenticated checkout did not render the saved delivery address');
  ...
  for (const expected of ['₹250', '₹45', '₹49', '₹344']) if (!authenticatedPriceText.includes(expected)) throw new Error(`authenticated checkout price summary missing ${expected}`);
  ...
  const completedState = await checkoutPage.evaluate(({ cartKey, checkoutKey }) => ({ cart: localStorage.getItem(cartKey), checkout: sessionStorage.getItem(checkoutKey) }), { cartKey: CART_KEY, checkoutKey: CHECKOUT_KEY });
  if (completedState.cart !== null || completedState.checkout !== null) throw new Error(`completed authenticated checkout did not clear state: cart=${completedState.cart}, checkout=${completedState.checkout}`);
