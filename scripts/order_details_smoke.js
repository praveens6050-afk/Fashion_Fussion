...
(async () => {
  ...
  const activeOrder = {
    ...
    items: [{
      id: 900003,
      name: 'Promotion Restore Product',
      ...
      (() => {
        ...
        function query(table){
          const q={
            ...
            async maybeSingle(){
              if(table==='orders'){
                const data=window.__smokeCancelled?{...order,status:'refund_initiated',fulfillment_status:'cancelled',cancellation_reason:'Ordered by mistake'}:order;
        ...
        document.addEventListener('DOMContentLoaded',()=>{
          ...
          s.src='order-cancel-promotion.js?v=1';
          s.async=false;
  ...
  await page.route('https://fashion-fussion-olive.vercel.app/api/**', async route => {
    ...
    if (request.headers()['authorization'] !== 'Bearer smoke-access-token') {
      throw new Error('order cancellation API call did not include the current bearer session');
    }
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        promotions_restored: true,
        refund: {
          required: true,
          status: 'pending',
          amount: 245,
          delivery_non_refundable: 49,
  ...
  const actionText = await page.locator('#actionsSection').textContent();
  if (!actionText.includes('same gift card')) throw new Error('gift-card cancellation UI did not explain balance restoration');
  ...
  for (const expected of ['Gift card balance restoration', '₹50', 'original payment method', 'same gift card']) {
    if (!previewText.includes(expected)) throw new Error(`gift-card cancellation preview missing ${expected}`);
  }
  if (!previewText.includes('₹49')) throw new Error('gift-card cancellation preview did not preserve non-refundable delivery');

  await page.locator('#cancelReason').selectOption({ label: 'Ordered by mistake' });
  await page.locator('#confirmCancel').click();
  await page.waitForFunction(() => window.location.pathname.endsWith('/order-details.html'), null, { timeout: 10000 });
  ...
  await context.close();
  await browser.close();
  console.log('Authenticated order-details cancellation smoke passed. Gift-card restoration messaging and cancellation submission are covered.');
})().catch(error => {
