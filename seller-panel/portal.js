...
function statusTag(status){const labels={new:'New',processing:'Processing',shipped:'Shipped',completed:'Completed',cancelled:'Cancelled',paid:'Paid',pending:'Pending',return:'Return requested'};return`<span class="tag ${status}">${STATUS_LABELS?.[status]||labels[status]||status}</span>`}
