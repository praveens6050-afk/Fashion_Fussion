...
- Homepage customer navigation remains discoverable through the existing business-registration trust runtime, which adds FAQ, Contact, Shipping, Returns, Terms, Privacy, About and Business Buying links without exposing private certificate/address data.

## Regression protection

`scripts/check_public_launch_source.js` is chained into the existing checkout/readiness Quality Gate source-check step. It fails CI if a required public page disappears, drops out of the sitemap/homepage discovery contract, is blocked by robots, loses its canonical declaration, or if private-route indexing/security-header protections regress.
